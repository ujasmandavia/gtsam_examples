...
using symbol_shorthand::X;    //Smart stereo factor
using symbol_shorthand::V;    //Imu velocity
using symbol_shorthand::B;    //Imu biases

struct IMUHelper{
  IMUHelper(){
    {
      auto gaussian = noiseModel::Diagonal::Sigmas((Vector(6) << Vector3::Constant(5.0e-2),Vector3::Constant(5.0e-3)).
                                                    finished());
    ...
    //expect IMU to be rotated in image space co-ordinates
...
int main(int argc, char* argv[]){
  if (argc != 2) {
    std::cout << "./ISAM2_SmartFactorStereo_IMU [data.txt]\n";
    return 0;
  }
  ...
  while(true){
    ...
    if(type == 'i'){
      ...
      if(smartFactors.count(landmark) == 0){
        ...
        graph.add(smartFactors[landmark]);
      }
